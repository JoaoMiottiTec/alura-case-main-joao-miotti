...
class CourseControllerTest {
    ...
    void editForm__renders_edit_page_with_form_and_combos() throws Exception {
        ...
        mockMvc.perform(get("/admin/course/{code}/edit", "java-web"))
                .andExpect(status().isOk())
    ...
    void editForm__redirects_to_list_when_course_not_found() throws Exception {
        when(service.findByCodeOrThrow("nope")).thenThrow(new IllegalArgumentException("Course not found"));

        mockMvc.perform(get("/admin/course/{code}/edit", "nope"))
                .andExpect(status().is3xxRedirection())
                .andExpect(redirectedUrl("/admin/courses"));
    }
    ...
    void edit__redirects_on_success() throws Exception {
        when(service.update(anyString(), any(UpdateCourseForm.class)))
                .thenReturn(new CourseDTO(1L, "Java Web", "java-web",
                        "ana@alura.com", 1L, "Back-End", "desc", "ACTIVE"));

        mockMvc.perform(post("/admin/course/{code}/edit", "java-web")
                        .param("name", "Java Web")
                        .param("instructorEmail", "ana@alura.com")
                        .param("categoryId", "1")
