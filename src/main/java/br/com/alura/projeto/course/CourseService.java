...
public class CourseService {
  ...
  public CourseService(
      CourseRepository courses, UserRepository users, CategoryRepository categories) {
  ...
  public List<CourseDTO> listAll() {
    return courses.findAll().stream().map(CourseDTO::new).toList();
  }
  ...
  public CourseDTO create(NewCourseForm form) {
    ...
    User instructor =
        users
            .findByEmail(form.getInstructorEmail())
            .orElseThrow(
                () -> new IllegalArgumentException("Instructor not found for the given email."));
  ...
  public void inactivateByCode(String code) {
    Course course =
        courses
            .findByCode(code)
            .orElseThrow(
                () -> new IllegalArgumentException("Course not found for the given code."));
  ...
  public CourseDTO update(String code, UpdateCourseForm form) {
    ...
    User instructor =
        users
            .findByEmail(form.getInstructorEmail().trim())
            .orElseThrow(
                () -> new IllegalArgumentException("Instructor not found for the given email."));
    if (instructor.getRole() != Role.INSTRUCTOR) {
      throw new IllegalArgumentException("User is not an INSTRUCTOR.");
